...
async function getApiClient() {
    if (!apiToken) {
        ...
        const resp = await axios_1.default.post(`${API_BASE}/auth/login`, form.toString(), {
...
const server = new index_js_1.Server({ name: "umpire-assignment-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
    ...
    try {
        switch (name) {
            ...
            case "respond_to_assignment": {
                const { assignment_id, action } = args;
